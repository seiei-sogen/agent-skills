---
name: outline
description: Progressive Outlining の第1段階。機能の粗い構造（EARS 要件・スコープ・ディレクトリ構成・モジュール境界）をユーザーと議論しながら docs/po/<slug>/01-outline.md に作る。コードは書かない。
...
成果物: `docs/po/$slug/01-outline.md`

## 進め方

1. `docs/po/$slug/` が既にあれば全ファイルを読み、前回の続きから始める。批評ファイルがあれば、未対処の指摘を先に列挙する。
