...
**リクエストのサーフェスとしてのMR：いいえ。** _(このリポジトリが外部マージリクエストを機能リクエストとして扱う場合は`yes`に設定してください；`/triage-matt-p`がこのフラグを読み取ります。)_
...
`/wayfinder-matt-p`で使用されます。**マップ**は単一の課題で、**子**課題がチケットとして存在します。
