...
このファイルは `pipe-requirements-to-pr` の Phase 5、Phase 8、Phase 10 で参照する。
...
最大3巡とする。重大または高重要度の所見が残る場合は、回数だけを理由に Phase 10 の最終更新へ進まない。仕様判断を要するが安全側の推奨案を採れる場合は、要件と計画を更新して反映する。

## commit、push、PR 前の状態確認

初回 PR は文書の品質ゲートが完了した Phase 5 で作成する。実装、レビューガイド、最終検証が完了した Phase 10 では、新しい PR を重ねて作らず、同じ PR に最終差分を追加して本文を更新する。
...
Phase 5 の初期タイトルと本文は、要件定義書、実装計画、品質ゲートの結果から作る。実装以降の工程が未完了であることも明記する。Phase 10 では最終差分、検証結果、レビューガイドを反映し、`pr-update-gh` を使って本文を整える。
